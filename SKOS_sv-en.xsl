...
<br />
				...
				<xsl:for-each select="//skos:prefLabel[@xml:lang='sv'] | //skos:altLabel">
	...
	<xsl:template match="skos:prefLabel[@xml:lang='sv']">
...
<div  class="uppslag">
				...
				<xsl:if test="../skos:prefLabel[@xml:lang='en']">
				<h4>
		...
		<xsl:for-each select="//skos:Concept[@rdf:about=$namn]">
			<xsl:choose>
				<xsl:when test="starts-with($namn, '#')">
					<a href="{$namn}">
						<xsl:value-of select="skos:prefLabel[@xml:lang='sv']"/>
						<xsl:if test="skos:prefLabel[@xml:lang='en']">&#47;<xsl:value-of select="skos:prefLabel[@xml:lang='en']"/></xsl:if>
					</a>
				</xsl:when>
